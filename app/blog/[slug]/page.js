...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: `/og?v=2&title=${encodeURIComponent(article.title)}&sub=${encodeURIComponent(article.category)}&tag=Blog` }],
    },
    twitter: {
...
export default async function ArticlePage({ params }) {
  ...
  const jsonLd = {
    ...
    image: `https://www.digitalcredityield.com/og?v=2&title=${encodeURIComponent(article.title)}&sub=${encodeURIComponent(article.category)}&tag=Blog`,
    url: `https://www.digitalcredityield.com/blog/${slug}`,
    author: {
      '@type': 'Person',
