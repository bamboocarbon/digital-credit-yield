...
const faqs = [
  ...
  },
  {
    ...
    a: 'STRC and SATA are designed to trade near their $100 par value: the issuers review the dividend rate each month, nudging it up if the price drifts below par to attract buyers, or down if it trades above. CHAD works the same way but against a $10 par value instead, with management reviewing the rate monthly or more often. BMNP works differently — its 9.50% rate is fixed, so instead of a moving rate it leans on a redemption premium (the company can buy shares back above par in the early years) to keep the price from drifting too far. Either way, prices tend to move in cents rather than dollars — and the effective yield you lock in depends on the price you pay relative to that instrument’s own par value.',
