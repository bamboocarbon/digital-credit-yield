...
export async function getChartData(ticker, period = '6mo', interval = '1d') {
  ...
  return result.quotes.map(q => ({
    ...
    // bar before it finalises the day's data; fall back to open so the bar still
    // renders instead of being silently dropped by the candlestick filter.
    close: q.close ?? q.open,
    volume: q.volume,
  }));
}
