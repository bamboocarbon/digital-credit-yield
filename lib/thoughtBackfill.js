...
// vxtwitter.com's Cloudflare rules now 403 the bare literal string
// "Mozilla/5.0" (a known naive-bot signature) specifically — confirmed
// 2026-09-14 by reproducing the block with curl/node fetch outside prod,
// while an honest identifying UA (or no UA at all) passes fine. This is
// what silently broke the 2026-09-13 entry's auto text/image backfill.
export async function fetchTweetImageUrl(tweetId) {
  ...
  const res = await fetch(`https://api.vxtwitter.com/${handle}/status/${tweetId}`, {
    headers: { 'User-Agent': 'DCYThoughtBot/1.0 (+https://digitalcredityield.com)' },
  });
