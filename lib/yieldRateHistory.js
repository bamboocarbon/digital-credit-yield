...
export const YIELD_RATE_HISTORY = {
  ...
  STRC: [
    ...
    { from: '2026-07-01', annualRate: 12.00 }, // first semi-monthly period at the new rate; confirmed via Strategy IR dividends page (July 2026 #1, record 07/15/2026)
  ],
  ...
  // Source: Bitmine 8-K announcing the Series A Preferred — fixed rate, no adjustments expected
  BMNP: [
    { from: '2026-06-16', annualRate: 9.50 }, // listing date
  ],
  // Source: DeFi Development Corp. FWP/424B5 pricing supplements for the Variable Rate
  // Series C Perpetual Preferred ("CHAD Stock") — management can adjust the rate to keep
  // the price within a stated long-term range, reviewed monthly or more often.
  CHAD: [
    { from: '2026-09-08', annualRate: 13.00 }, // IPO closing date
  ],
};
...
export function getRateForDate(ticker, dateStr) {
  ...
  const eligible = history.filter(e => e.from <= dateStr);
  if (eligible.length === 0) return null;
  return eligible.reduce((latest, e) => (e.from > latest.from ? e : latest)).annualRate;
}
